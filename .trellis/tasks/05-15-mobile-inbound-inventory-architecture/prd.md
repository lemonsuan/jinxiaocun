...
* App 入库单必须支持可选商家单号和返利单号；OCR 文本中的“订单号”应填入商家单号，不作为必填快递单号。
...
* App 出库单支持可选物流单号，用于历史出库展示和搜索，不影响库存扣减。
...
* [ ] 用户可在入库单填写或自动带入商家单号，并手动填写返利单号，二者为空时不阻塞入库。
* [ ] 用户可在商品总量中将商品加入出库购物车，并从购物车生成出库单。
* [ ] 用户可给一张出库单保存多张照片附件，并在历史出库中查看。
* [ ] 用户可给出库单填写物流单号，并在历史出库中搜索和查看。
